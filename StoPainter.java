...
import raf.draft.dsw.model.elements.Sto;

import java.awt.*;
import java.awt.geom.AffineTransform;

@Getter@Setter
public class StoPainter implements ElementPainter{
    Sto draftNode;
    Rectangle hitbox;
    boolean zakacen;
    Color color= MainFrame.getInstance().getBackground();
    public StoPainter(Sto draftNode) {
    ...
    public void paint(Graphics2D graphics) {
        ...
        int[][] points = {
                ...
                {x-width/2 + width - offset, y-height/2 + offset},
                {x-width/2+ offset, y-height/2 + height - offset},
                {x-width/2 + width - offset, y -height/2+ height - offset}
        };
